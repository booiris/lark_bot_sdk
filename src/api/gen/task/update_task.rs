//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/task-v1/task/patch>
...
impl<'c, IStore: Store, IClient: HttpClient> TaskService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/task-v1/task/patch>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/task-v1/task/patch>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Ftask-v1%2Ftask%2Fpatch>
    pub async fn update_task(
        &self,
        req: UpdateTaskReq,
    ) -> Result<(UpdateTaskResp, CommonResponse), Error> {
        self.update_task_with_opt(req, Default::default()).await
    }

    /// 参见 [update_task](#method.update_task) 函数
    pub async fn update_task_with_opt(
        &self,
        req: UpdateTaskReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_update_task(&req) {
                tracing::info!("[lark] Task#UpdateTask **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Task#UpdateTask call api");
        ...
        let (resp, common_resp): (UpdateTaskRespInner, _) = self.cli.do_req(req).await?;
...
pub struct TaskSubReq {
    ...
    pub origin: Option<OriginSubReq>,
    ...
    pub followers: Vec<Option<FollowerSubReq>>,
    /// 任务的执行者
    ///
    ...
    pub collaborators: Vec<Option<CollaboratorSubReq>>,
    /// 创建任务时添加的执行者用户id列表。
    ///
...
struct UpdateTaskRespInner {
    #[serde(flatten)]
    data: Option<UpdateTaskResp>,
    #[serde(flatten)]
    base: BaseResp,
}
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(UpdateTaskReq) -> Result<(UpdateTaskResp, CommonResponse), Error> + Send + Sync + 'static
    {
    }
    impl<
            T: Fn(UpdateTaskReq) -> Result<(UpdateTaskResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> TaskServiceMocker<'c, IStore, IClient> {
        pub fn mock_update_task<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, UpdateTaskReq, UpdateTaskResp, Arc<dyn MockFunc>> {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_update_task(
            &self,
            req: &UpdateTaskReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, UpdateTaskReq, UpdateTaskResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::task::update_task::{UpdateTaskReq, UpdateTaskResp, UpdateTaskRespInner},
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .task()
            .mock()
            .mock_update_task(|_| Ok((UpdateTaskResp::default(), CommonResponse::default())))
            .build();
        let res = lark.task().update_task(UpdateTaskReq::default()).await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark.task().update_task(UpdateTaskReq::default()).await;
        assert!(res.is_err());
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::UpdateTaskReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<UpdateTaskRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
