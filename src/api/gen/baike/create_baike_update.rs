//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/baike-v1/draft/update>
...
impl<'c, IStore: Store, IClient: HttpClient> BaikeService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/baike-v1/draft/update>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/baike-v1/draft/update>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fbaike-v1%2Fdraft%2Fupdate>
    pub async fn create_baike_update(
        &self,
        req: CreateBaikeUpdateReq,
    ) -> Result<(CreateBaikeUpdateResp, CommonResponse), Error> {
        self.create_baike_update_with_opt(req, Default::default())
            .await
    }

    /// 参见 [create_baike_update](#method.create_baike_update) 函数
    pub async fn create_baike_update_with_opt(
        &self,
        req: CreateBaikeUpdateReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_create_baike_update(&req) {
                tracing::info!("[lark] Baike#CreateBaikeUpdate **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Baike#CreateBaikeUpdate call api");
        ...
        let (resp, common_resp): (CreateBaikeUpdateRespInner, _) = self.cli.do_req(req).await?;
...
pub struct CreateBaikeUpdateReq {
    /// 草稿 ID
    ///
    /// **是否必填**: 是
    ///
    /// **示例值**: "5347"
    #[api(kind = "path", name = "draft_id")]
    pub draft_id: String,
    /// 用户 ID 类型
    ///
    /// **示例值**: "open_id"
...
struct CreateBaikeUpdateRespInner {
    #[serde(flatten)]
    data: Option<CreateBaikeUpdateResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct CreateBaikeUpdateResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(CreateBaikeUpdateReq) -> Result<(CreateBaikeUpdateResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(CreateBaikeUpdateReq) -> Result<(CreateBaikeUpdateResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> BaikeServiceMocker<'c, IStore, IClient> {
        pub fn mock_create_baike_update<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, CreateBaikeUpdateReq, CreateBaikeUpdateResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_create_baike_update(
            &self,
            req: &CreateBaikeUpdateReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, CreateBaikeUpdateReq, CreateBaikeUpdateResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::baike::create_baike_update::{
            CreateBaikeUpdateReq, CreateBaikeUpdateResp, CreateBaikeUpdateRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .baike()
            .mock()
            .mock_create_baike_update(|_| {
                Ok((CreateBaikeUpdateResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .baike()
            .create_baike_update(CreateBaikeUpdateReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::CreateBaikeUpdateReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<CreateBaikeUpdateRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
