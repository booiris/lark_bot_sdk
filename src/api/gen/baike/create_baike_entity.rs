//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/baike-v1/entity/create>
...
impl<'c, IStore: Store, IClient: HttpClient> BaikeService<'c, IStore, IClient> {
    /// **api 版本: 2024-04-24T07:19:22+00:00**
    ///
    /// ## 创建免审词条
    ///
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/baike-v1/entity/create>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/baike-v1/entity/create>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fbaike-v1%2Fentity%2Fcreate>
    pub async fn create_baike_entity(
        &self,
        req: CreateBaikeEntityReq,
    ) -> Result<(CreateBaikeEntityResp, CommonResponse), Error> {
        self.create_baike_entity_with_opt(req, Default::default())
            .await
    }

    /// 参见 [create_baike_entity](#method.create_baike_entity) 函数
    pub async fn create_baike_entity_with_opt(
        &self,
        req: CreateBaikeEntityReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_create_baike_entity(&req) {
                tracing::info!("[lark] Baike#CreateBaikeEntity **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Baike#CreateBaikeEntity call api");
        ...
        let (resp, common_resp): (CreateBaikeEntityRespInner, _) = self.cli.do_req(req).await?;
...
pub struct CreateBaikeEntityReq {
    /// 用户 ID 类型
    ///
    /// **示例值**: "open_id"
    ///
...
struct CreateBaikeEntityRespInner {
    #[serde(flatten)]
    data: Option<CreateBaikeEntityResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct CreateBaikeEntityResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(CreateBaikeEntityReq) -> Result<(CreateBaikeEntityResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(CreateBaikeEntityReq) -> Result<(CreateBaikeEntityResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> BaikeServiceMocker<'c, IStore, IClient> {
        pub fn mock_create_baike_entity<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, CreateBaikeEntityReq, CreateBaikeEntityResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_create_baike_entity(
            &self,
            req: &CreateBaikeEntityReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, CreateBaikeEntityReq, CreateBaikeEntityResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::baike::create_baike_entity::{
            CreateBaikeEntityReq, CreateBaikeEntityResp, CreateBaikeEntityRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .baike()
            .mock()
            .mock_create_baike_entity(|_| {
                Ok((CreateBaikeEntityResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .baike()
            .create_baike_entity(CreateBaikeEntityReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::CreateBaikeEntityReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<CreateBaikeEntityRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
