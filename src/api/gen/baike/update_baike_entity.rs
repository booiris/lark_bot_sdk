//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/baike-v1/entity/update>
...
impl<'c, IStore: Store, IClient: HttpClient> BaikeService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/baike-v1/entity/update>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/baike-v1/entity/update>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fbaike-v1%2Fentity%2Fupdate>
    pub async fn update_baike_entity(
        &self,
        req: UpdateBaikeEntityReq,
    ) -> Result<(UpdateBaikeEntityResp, CommonResponse), Error> {
        self.update_baike_entity_with_opt(req, Default::default())
            .await
    }

    /// 参见 [update_baike_entity](#method.update_baike_entity) 函数
    pub async fn update_baike_entity_with_opt(
        &self,
        req: UpdateBaikeEntityReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_update_baike_entity(&req) {
                tracing::info!("[lark] Baike#UpdateBaikeEntity **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Baike#UpdateBaikeEntity call api");
        ...
        let (resp, common_resp): (UpdateBaikeEntityRespInner, _) = self.cli.do_req(req).await?;
...
pub struct UpdateBaikeEntityReq {
    /// 实体词 ID
    ///
...
struct UpdateBaikeEntityRespInner {
    #[serde(flatten)]
    data: Option<UpdateBaikeEntityResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct UpdateBaikeEntityResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(UpdateBaikeEntityReq) -> Result<(UpdateBaikeEntityResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(UpdateBaikeEntityReq) -> Result<(UpdateBaikeEntityResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> BaikeServiceMocker<'c, IStore, IClient> {
        pub fn mock_update_baike_entity<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, UpdateBaikeEntityReq, UpdateBaikeEntityResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_update_baike_entity(
            &self,
            req: &UpdateBaikeEntityReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, UpdateBaikeEntityReq, UpdateBaikeEntityResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::baike::update_baike_entity::{
            UpdateBaikeEntityReq, UpdateBaikeEntityResp, UpdateBaikeEntityRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .baike()
            .mock()
            .mock_update_baike_entity(|_| {
                Ok((UpdateBaikeEntityResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .baike()
            .update_baike_entity(UpdateBaikeEntityReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::UpdateBaikeEntityReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<UpdateBaikeEntityRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
