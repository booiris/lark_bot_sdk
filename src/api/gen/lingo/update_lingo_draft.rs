//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/lingo-v1/draft/update>
...
impl<'c, IStore: Store, IClient: HttpClient> LingoService<'c, IStore, IClient> {
    /// **api 版本: 2023-10-25T08:25:17+00:00**
    ///
    /// ## 更新草稿
    ///
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/lingo-v1/draft/update>
    ///
    /// new doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/lingo-v1/draft/update>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2FuAjLw4CM%2FukTMukTMukTM%2Flingo-v1%2Fdraft%2Fupdate>
    pub async fn update_lingo_draft(
        &self,
        req: UpdateLingoDraftReq,
    ) -> Result<(UpdateLingoDraftResp, CommonResponse), Error> {
        self.update_lingo_draft_with_opt(req, Default::default())
            .await
    }

    /// 参见 [update_lingo_draft](#method.update_lingo_draft) 函数
    pub async fn update_lingo_draft_with_opt(
        &self,
        req: UpdateLingoDraftReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_update_lingo_draft(&req) {
                tracing::info!("[lark] Lingo#UpdateLingoDraft **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Lingo#UpdateLingoDraft call api");
        ...
        let (resp, common_resp): (UpdateLingoDraftRespInner, _) = self.cli.do_req(req).await?;
...
pub struct UpdateLingoDraftReq {
    /// 草稿ID
    ///
    /// **是否必填**: 是
    ///
...
struct UpdateLingoDraftRespInner {
    #[serde(flatten)]
    data: Option<UpdateLingoDraftResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct UpdateLingoDraftResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(UpdateLingoDraftReq) -> Result<(UpdateLingoDraftResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(UpdateLingoDraftReq) -> Result<(UpdateLingoDraftResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> LingoServiceMocker<'c, IStore, IClient> {
        pub fn mock_update_lingo_draft<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, UpdateLingoDraftReq, UpdateLingoDraftResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_update_lingo_draft(
            &self,
            req: &UpdateLingoDraftReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, UpdateLingoDraftReq, UpdateLingoDraftResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::lingo::update_lingo_draft::{
            UpdateLingoDraftReq, UpdateLingoDraftResp, UpdateLingoDraftRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .lingo()
            .mock()
            .mock_update_lingo_draft(|_| {
                Ok((UpdateLingoDraftResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .lingo()
            .update_lingo_draft(UpdateLingoDraftReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::UpdateLingoDraftReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<UpdateLingoDraftRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
