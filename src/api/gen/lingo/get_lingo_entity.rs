//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/lingo-v1/entity/get>
...
impl<'c, IStore: Store, IClient: HttpClient> LingoService<'c, IStore, IClient> {
    /// **api 版本: 2023-10-25T08:25:53+00:00**
    ///
    /// ## 获取词条详情
    ///
    /// 通过词条 id 拉取对应的词条详情信息。
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/lingo-v1/entity/get>
    ///
    /// new doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/lingo-v1/entity/get>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2FuAjLw4CM%2FukTMukTMukTM%2Flingo-v1%2Fentity%2Fget>
    pub async fn get_lingo_entity(
        &self,
        req: GetLingoEntityReq,
    ) -> Result<(GetLingoEntityResp, CommonResponse), Error> {
        self.get_lingo_entity_with_opt(req, Default::default())
            .await
    }

    /// 参见 [get_lingo_entity](#method.get_lingo_entity) 函数
    pub async fn get_lingo_entity_with_opt(
        &self,
        req: GetLingoEntityReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_get_lingo_entity(&req) {
                tracing::info!("[lark] Lingo#GetLingoEntity **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Lingo#GetLingoEntity call api");
        ...
        let (resp, common_resp): (GetLingoEntityRespInner, _) = self.cli.do_req(req).await?;
...
struct GetLingoEntityRespInner {
    #[serde(flatten)]
    data: Option<GetLingoEntityResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct GetLingoEntityResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(GetLingoEntityReq) -> Result<(GetLingoEntityResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(GetLingoEntityReq) -> Result<(GetLingoEntityResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> LingoServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_lingo_entity<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, GetLingoEntityReq, GetLingoEntityResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_get_lingo_entity(
            &self,
            req: &GetLingoEntityReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, GetLingoEntityReq, GetLingoEntityResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::lingo::get_lingo_entity::{
            GetLingoEntityReq, GetLingoEntityResp, GetLingoEntityRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .lingo()
            .mock()
            .mock_get_lingo_entity(|_| {
                Ok((GetLingoEntityResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .lingo()
            .get_lingo_entity(GetLingoEntityReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetLingoEntityRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
