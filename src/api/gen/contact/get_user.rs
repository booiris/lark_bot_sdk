//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/get>
...
impl<'c, IStore: Store, IClient: HttpClient> ContactService<'c, IStore, IClient> {
    /// **api 版本: 2024-08-01T09:24:33+00:00**
    ///
    /// ## 获取单个用户信息
    ///
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/get>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/contact-v3/user/get>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fcontact-v3%2Fuser%2Fget>
    pub async fn get_user(&self, req: GetUserReq) -> Result<(GetUserResp, CommonResponse), Error> {
        self.get_user_with_opt(req, Default::default()).await
    }

    /// 参见 [get_user](#method.get_user) 函数
    pub async fn get_user_with_opt(
        &self,
        req: GetUserReq,
        method_option: MethodOption,
    ) -> Result<(GetUserResp, CommonResponse), Error> {
        #[cfg(feature = "test-util")]
        {
            if let Some(f) = self.mock().get_mock_get_user(&req) {
                tracing::info!("[lark] Contact#GetUser **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Contact#GetUser call api");
        ...
        let (resp, common_resp): (GetUserRespInner, _) = self.cli.do_req(req).await?;
...
struct GetUserRespInner {
    #[serde(flatten)]
    data: Option<GetUserResp>,
    #[serde(flatten)]
    base: BaseResp,
}
...
pub struct UserSubResp {
    ...
    /// 部门路径列表。该字段需要通过用户身份（user_access_token）调用接口才可以获取。
    #[serde(
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(GetUserReq) -> Result<(GetUserResp, CommonResponse), Error> + Send + Sync + 'static
    {
    }
    impl<
            T: Fn(GetUserReq) -> Result<(GetUserResp, CommonResponse), Error> + Send + Sync + 'static,
        > MockFunc for T
    {
    }

    impl<'c, IStore: Store, IClient: HttpClient> ContactServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_user<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, GetUserReq, GetUserResp, Arc<dyn MockFunc>> {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_get_user(&self, req: &GetUserReq) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, GetUserReq, GetUserResp, Arc<dyn MockFunc>>(self.cli.instance_id, req)
        }
    }
}
...
mod test {
    use crate::{
        api::gen::contact::get_user::{GetUserReq, GetUserResp, GetUserRespInner},
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .contact()
            .mock()
            .mock_get_user(|_| Ok((GetUserResp::default(), CommonResponse::default())))
            .build();
        let res = lark.contact().get_user(GetUserReq::default()).await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark.contact().get_user(GetUserReq::default()).await;
        assert!(res.is_err());
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetUserRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
