//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/get>
...
impl<'c, IStore: Store, IClient: HttpClient> ContactService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/get>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/contact-v3/department/get>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fcontact-v3%2Fdepartment%2Fget>
    pub async fn get_department(
        &self,
        req: GetDepartmentReq,
    ) -> Result<(GetDepartmentResp, CommonResponse), Error> {
        self.get_department_with_opt(req, Default::default()).await
    }

    /// 参见 [get_department](#method.get_department) 函数
    pub async fn get_department_with_opt(
        &self,
        req: GetDepartmentReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_get_department(&req) {
                tracing::info!("[lark] Contact#GetDepartment **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Contact#GetDepartment call api");
        ...
        let (resp, common_resp): (GetDepartmentRespInner, _) = self.cli.do_req(req).await?;
...
struct GetDepartmentRespInner {
    #[serde(flatten)]
    data: Option<GetDepartmentResp>,
    #[serde(flatten)]
    base: BaseResp,
}
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(GetDepartmentReq) -> Result<(GetDepartmentResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(GetDepartmentReq) -> Result<(GetDepartmentResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> ContactServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_department<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, GetDepartmentReq, GetDepartmentResp, Arc<dyn MockFunc>> {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_get_department(
            &self,
            req: &GetDepartmentReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, GetDepartmentReq, GetDepartmentResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::contact::get_department::{
            GetDepartmentReq, GetDepartmentResp, GetDepartmentRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .contact()
            .mock()
            .mock_get_department(|_| Ok((GetDepartmentResp::default(), CommonResponse::default())))
            .build();
        let res = lark
            .contact()
            .get_department(GetDepartmentReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetDepartmentRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
