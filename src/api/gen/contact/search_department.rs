//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/search>
...
impl<'c, IStore: Store, IClient: HttpClient> ContactService<'c, IStore, IClient> {
    /// **api 版本: 2024-07-05T08:11:28+00:00**
    ///
    /// ## 搜索部门
    ///
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/search>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/contact-v3/department/search>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fcontact-v3%2Fdepartment%2Fsearch>
    pub async fn search_department(
        &self,
        req: SearchDepartmentReq,
    ) -> Result<(SearchDepartmentResp, CommonResponse), Error> {
        self.search_department_with_opt(req, Default::default())
            .await
    }

    /// 参见 [search_department](#method.search_department) 函数
    pub async fn search_department_with_opt(
        &self,
        req: SearchDepartmentReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_search_department(&req) {
                tracing::info!("[lark] Contact#SearchDepartment **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Contact#SearchDepartment call api");
        ...
        let (resp, common_resp): (SearchDepartmentRespInner, _) = self.cli.do_req(req).await?;
...
pub struct SearchDepartmentReq {
    /// 用户 ID 类型
    ///
    /// **示例值**: "open_id"
    ///
...
struct SearchDepartmentRespInner {
    #[serde(flatten)]
    data: Option<SearchDepartmentResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct SearchDepartmentResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(SearchDepartmentReq) -> Result<(SearchDepartmentResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(SearchDepartmentReq) -> Result<(SearchDepartmentResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> ContactServiceMocker<'c, IStore, IClient> {
        pub fn mock_search_department<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, SearchDepartmentReq, SearchDepartmentResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_search_department(
            &self,
            req: &SearchDepartmentReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, SearchDepartmentReq, SearchDepartmentResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::contact::search_department::{
            SearchDepartmentReq, SearchDepartmentResp, SearchDepartmentRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .contact()
            .mock()
            .mock_search_department(|_| {
                Ok((SearchDepartmentResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .contact()
            .search_department(SearchDepartmentReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::SearchDepartmentReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<SearchDepartmentRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
