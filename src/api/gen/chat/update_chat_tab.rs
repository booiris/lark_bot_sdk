//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/update_tabs>
...
impl<'c, IStore: Store, IClient: HttpClient> ChatService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-tab/update_tabs>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/group/chat-tab/update_tabs>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fgroup%2Fchat-tab%2Fupdate_tabs>
    pub async fn update_chat_tab(
        &self,
        req: UpdateChatTabReq,
    ) -> Result<(UpdateChatTabResp, CommonResponse), Error> {
        self.update_chat_tab_with_opt(req, Default::default()).await
    }

    /// 参见 [update_chat_tab](#method.update_chat_tab) 函数
    pub async fn update_chat_tab_with_opt(
        &self,
        req: UpdateChatTabReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_update_chat_tab(&req) {
                tracing::info!("[lark] Chat#UpdateChatTab **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Chat#UpdateChatTab call api");
        ...
        let (resp, common_resp): (UpdateChatTabRespInner, _) = self.cli.do_req(req).await?;
...
struct UpdateChatTabRespInner {
    #[serde(flatten)]
    data: Option<UpdateChatTabResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct UpdateChatTabResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(UpdateChatTabReq) -> Result<(UpdateChatTabResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(UpdateChatTabReq) -> Result<(UpdateChatTabResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> ChatServiceMocker<'c, IStore, IClient> {
        pub fn mock_update_chat_tab<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, UpdateChatTabReq, UpdateChatTabResp, Arc<dyn MockFunc>> {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_update_chat_tab(
            &self,
            req: &UpdateChatTabReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, UpdateChatTabReq, UpdateChatTabResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::chat::update_chat_tab::{
            UpdateChatTabReq, UpdateChatTabResp, UpdateChatTabRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .chat()
            .mock()
            .mock_update_chat_tab(|_| Ok((UpdateChatTabResp::default(), CommonResponse::default())))
            .build();
        let res = lark
            .chat()
            .update_chat_tab(UpdateChatTabReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::UpdateChatTabReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<UpdateChatTabRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
