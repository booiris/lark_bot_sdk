//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-menu_tree/get>
...
impl<'c, IStore: Store, IClient: HttpClient> ChatService<'c, IStore, IClient> {
    ...
    /// - 机器人必须在群里。
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-menu_tree/get>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/group/chat-menu_tree/get>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fgroup%2Fchat-menu_tree%2Fget>
    pub async fn get_chat_menu_tree(
        &self,
        req: GetChatMenuTreeReq,
    ) -> Result<(GetChatMenuTreeResp, CommonResponse), Error> {
        self.get_chat_menu_tree_with_opt(req, Default::default())
            .await
    }

    /// 参见 [get_chat_menu_tree](#method.get_chat_menu_tree) 函数
    pub async fn get_chat_menu_tree_with_opt(
        &self,
        req: GetChatMenuTreeReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_get_chat_menu_tree(&req) {
                tracing::info!("[lark] Chat#GetChatMenuTree **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Chat#GetChatMenuTree call api");
        ...
        let (resp, common_resp): (GetChatMenuTreeRespInner, _) = self.cli.do_req(req).await?;
...
struct GetChatMenuTreeRespInner {
    #[serde(flatten)]
    data: Option<GetChatMenuTreeResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct GetChatMenuTreeResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(GetChatMenuTreeReq) -> Result<(GetChatMenuTreeResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(GetChatMenuTreeReq) -> Result<(GetChatMenuTreeResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> ChatServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_chat_menu_tree<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, GetChatMenuTreeReq, GetChatMenuTreeResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_get_chat_menu_tree(
            &self,
            req: &GetChatMenuTreeReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, GetChatMenuTreeReq, GetChatMenuTreeResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::chat::get_chat_menu_tree::{
            GetChatMenuTreeReq, GetChatMenuTreeResp, GetChatMenuTreeRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .chat()
            .mock()
            .mock_get_chat_menu_tree(|_| {
                Ok((GetChatMenuTreeResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .chat()
            .get_chat_menu_tree(GetChatMenuTreeReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetChatMenuTreeRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
