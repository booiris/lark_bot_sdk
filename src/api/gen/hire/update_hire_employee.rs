//! doc url: <https://open.larkoffice.com/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/employee/patch>
...
impl<'c, IStore: Store, IClient: HttpClient> HireService<'c, IStore, IClient> {
    /// **api 版本: 2024-01-22T08:34:19+00:00**
    ///
    /// ## 更新入职状态
    ///
    /// 根据员工 ID 更新员工转正、离职状态。
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/ukTMukTMukTM/uMzM1YjLzMTN24yMzUjN/hire-v1/employee/patch>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/hire-v1/candidate-management/delivery-process-management/onboard/patch>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fhire-v1%2Fcandidate-management%2Fdelivery-process-management%2Fonboard%2Fpatch>
    pub async fn update_hire_employee(
        &self,
        req: UpdateHireEmployeeReq,
    ) -> Result<(UpdateHireEmployeeResp, CommonResponse), Error> {
        self.update_hire_employee_with_opt(req, Default::default())
            .await
    }

    /// 参见 [update_hire_employee](#method.update_hire_employee) 函数
    pub async fn update_hire_employee_with_opt(
        &self,
        req: UpdateHireEmployeeReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_update_hire_employee(&req) {
                tracing::info!("[lark] Hire#UpdateHireEmployee **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Hire#UpdateHireEmployee call api");
        ...
        let (resp, common_resp): (UpdateHireEmployeeRespInner, _) = self.cli.do_req(req).await?;
...
pub struct UpdateHireEmployeeReq {
    /// 员工ID
    ///
    /// **是否必填**: 是
    ///
    ...
    pub operation: i64,
    /// 转正信息，操作员工转正时必填
    #[api(kind = "body", name = "conversion_info")]
    pub conversion_info: Option<EmployeeConversionInfoSubReq>,
    /// 离职信息，操作员工离职时必填
    #[api(kind = "body", name = "overboard_info")]
    pub overboard_info: Option<EmployeeOverboardInfoSubReq>,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
#[serde(default)]
pub struct EmployeeConversionInfoSubReq {
    /// 实际转正日期
    ///
    /// **示例值**: "1637596800000"
    #[serde(
...
#[derive(Debug, Serialize, Deserialize, Clone, Default)]
#[serde(default)]
pub struct EmployeeOverboardInfoSubReq {
    /// 实际离职日期
    ///
    /// **示例值**: "1637596800000"
    #[serde(
        rename = "actual_overboard_time",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
    )]
    pub actual_overboard_time: Option<i64>,
    /// 离职原因
    ///
    /// **示例值**: "职业发展考虑"
    #[serde(
...
struct UpdateHireEmployeeRespInner {
    #[serde(flatten)]
    data: Option<UpdateHireEmployeeResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct UpdateHireEmployeeResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(UpdateHireEmployeeReq) -> Result<(UpdateHireEmployeeResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(UpdateHireEmployeeReq) -> Result<(UpdateHireEmployeeResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> HireServiceMocker<'c, IStore, IClient> {
        pub fn mock_update_hire_employee<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, UpdateHireEmployeeReq, UpdateHireEmployeeResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_update_hire_employee(
            &self,
            req: &UpdateHireEmployeeReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, UpdateHireEmployeeReq, UpdateHireEmployeeResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::hire::update_hire_employee::{
            UpdateHireEmployeeReq, UpdateHireEmployeeResp, UpdateHireEmployeeRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .hire()
            .mock()
            .mock_update_hire_employee(|_| {
                Ok((UpdateHireEmployeeResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .hire()
            .update_hire_employee(UpdateHireEmployeeReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::UpdateHireEmployeeReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<UpdateHireEmployeeRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
