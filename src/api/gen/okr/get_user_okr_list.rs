//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/user-okr/list>
...
impl<'c, IStore: Store, IClient: HttpClient> OkrService<'c, IStore, IClient> {
    /// **api 版本: 2024-04-25T03:56:17+00:00**
    ///
    /// ## 获取用户的 OKR 列表
    ///
    /// 根据用户的 id 获取 OKR 列表。
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/user-okr/list>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/okr-v1/okr/list>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fokr-v1%2Fokr%2Flist>
    pub async fn get_user_okr_list(
        &self,
        req: GetUserOkrListReq,
    ) -> Result<(GetUserOkrListResp, CommonResponse), Error> {
        self.get_user_okr_list_with_opt(req, Default::default())
            .await
    }

    /// 参见 [get_user_okr_list](#method.get_user_okr_list) 函数
    pub async fn get_user_okr_list_with_opt(
        &self,
        req: GetUserOkrListReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_get_user_okr_list(&req) {
                tracing::info!("[lark] Okr#GetUserOkrList **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Okr#GetUserOkrList call api");
        ...
        let (resp, common_resp): (GetUserOkrListRespInner, _) = self.cli.do_req(req).await?;
...
pub struct GetUserOkrListReq {
    ...
    ///
    /// **示例值**: "0"
    #[api(kind = "query", name = "offset", v_type = "var", option = "false")]
    pub offset: String,
    /// 列表长度，0-10
    ///
    /// **是否必填**: 是
    ...
    ///
    /// - **最大长度**: `10` 字符
    #[api(kind = "query", name = "period_ids", v_type = "list", option = "false")]
    pub period_ids: Vec<String>,
}

#[derive(Debug, Serialize, Deserialize, Clone, lark_bot_sdk_macros::ApiBaseResp)]
struct GetUserOkrListRespInner {
    #[serde(flatten)]
    data: Option<GetUserOkrListResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct GetUserOkrListResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(GetUserOkrListReq) -> Result<(GetUserOkrListResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(GetUserOkrListReq) -> Result<(GetUserOkrListResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> OkrServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_user_okr_list<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, GetUserOkrListReq, GetUserOkrListResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_get_user_okr_list(
            &self,
            req: &GetUserOkrListReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, GetUserOkrListReq, GetUserOkrListResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::okr::get_user_okr_list::{
            GetUserOkrListReq, GetUserOkrListResp, GetUserOkrListRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .okr()
            .mock()
            .mock_get_user_okr_list(|_| {
                Ok((GetUserOkrListResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .okr()
            .get_user_okr_list(GetUserOkrListReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetUserOkrListRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
