//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/progress_record/update>
...
impl<'c, IStore: Store, IClient: HttpClient> OkrService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/progress_record/update>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/okr-v1/progress_record/update>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fokr-v1%2Fprogress_record%2Fupdate>
    pub async fn update_okr_progress_record(
        &self,
        req: UpdateOkrProgressRecordReq,
    ) -> Result<(UpdateOkrProgressRecordResp, CommonResponse), Error> {
        self.update_okr_progress_record_with_opt(req, Default::default())
            .await
    }

    /// 参见 [update_okr_progress_record](#method.update_okr_progress_record) 函数
    pub async fn update_okr_progress_record_with_opt(
        &self,
        req: UpdateOkrProgressRecordReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_update_okr_progress_record(&req) {
                tracing::info!("[lark] Okr#UpdateOkrProgressRecord **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Okr#UpdateOkrProgressRecord call api");
        ...
        let (resp, common_resp): (UpdateOkrProgressRecordRespInner, _) =
...
pub struct UpdateOkrProgressRecordReq {
    /// 待更新的 OKR进展记录 ID
    ///
    /// **是否必填**: 是
    ///
...
struct UpdateOkrProgressRecordRespInner {
    #[serde(flatten)]
    data: Option<UpdateOkrProgressRecordResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct UpdateOkrProgressRecordResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    impl<'c, IStore: Store, IClient: HttpClient> OkrServiceMocker<'c, IStore, IClient> {
        pub fn mock_update_okr_progress_record<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<
            Mocker,
        ...
        pub(super) fn get_mock_update_okr_progress_record(
            &self,
            req: &UpdateOkrProgressRecordReq,
...
mod test {
    use crate::{
        api::gen::okr::update_okr_progress_record::{
            UpdateOkrProgressRecordReq, UpdateOkrProgressRecordResp,
            UpdateOkrProgressRecordRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .okr()
            .mock()
            .mock_update_okr_progress_record(|_| {
                Ok((
                    UpdateOkrProgressRecordResp::default(),
        ...
        let res = lark
            .okr()
            .update_okr_progress_record(UpdateOkrProgressRecordReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::UpdateOkrProgressRecordReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<UpdateOkrProgressRecordRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
