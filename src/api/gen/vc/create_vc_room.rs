//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/room/create>
...
impl<'c, IStore: Store, IClient: HttpClient> VcService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/vc-v1/room/create>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/vc-v1/room/create>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fvc-v1%2Froom%2Fcreate>
    pub async fn create_vc_room(
        &self,
        req: CreateVcRoomReq,
    ) -> Result<(CreateVcRoomResp, CommonResponse), Error> {
        self.create_vc_room_with_opt(req, Default::default()).await
    }

    /// 参见 [create_vc_room](#method.create_vc_room) 函数
    pub async fn create_vc_room_with_opt(
        &self,
        req: CreateVcRoomReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_create_vc_room(&req) {
                tracing::info!("[lark] Vc#CreateVcRoom **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Vc#CreateVcRoom call api");
        ...
        let (resp, common_resp): (CreateVcRoomRespInner, _) = self.cli.do_req(req).await?;
...
struct CreateVcRoomRespInner {
    #[serde(flatten)]
    data: Option<CreateVcRoomResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct CreateVcRoomResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(CreateVcRoomReq) -> Result<(CreateVcRoomResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(CreateVcRoomReq) -> Result<(CreateVcRoomResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> VcServiceMocker<'c, IStore, IClient> {
        pub fn mock_create_vc_room<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, CreateVcRoomReq, CreateVcRoomResp, Arc<dyn MockFunc>> {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_create_vc_room(
            &self,
            req: &CreateVcRoomReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, CreateVcRoomReq, CreateVcRoomResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::vc::create_vc_room::{CreateVcRoomReq, CreateVcRoomResp, CreateVcRoomRespInner},
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .vc()
            .mock()
            .mock_create_vc_room(|_| Ok((CreateVcRoomResp::default(), CommonResponse::default())))
            .build();
        let res = lark.vc().create_vc_room(CreateVcRoomReq::default()).await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark.vc().create_vc_room(CreateVcRoomReq::default()).await;
        assert!(res.is_err());
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::CreateVcRoomReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<CreateVcRoomRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
