//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/helpdesk-v1/ticket/get>
...
impl<'c, IStore: Store, IClient: HttpClient> HelpdeskService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/helpdesk-v1/ticket/get>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/helpdesk-v1/ticket-management/ticket/get>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fhelpdesk-v1%2Fticket-management%2Fticket%2Fget>
    pub async fn get_helpdesk_ticket(
        &self,
        req: GetHelpdeskTicketReq,
    ) -> Result<(GetHelpdeskTicketResp, CommonResponse), Error> {
        self.get_helpdesk_ticket_with_opt(req, Default::default())
            .await
    }

    /// 参见 [get_helpdesk_ticket](#method.get_helpdesk_ticket) 函数
    pub async fn get_helpdesk_ticket_with_opt(
        &self,
        req: GetHelpdeskTicketReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_get_helpdesk_ticket(&req) {
                tracing::info!("[lark] Helpdesk#GetHelpdeskTicket **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Helpdesk#GetHelpdeskTicket call api");
        ...
        let (resp, common_resp): (GetHelpdeskTicketRespInner, _) = self.cli.do_req(req).await?;
...
pub struct GetHelpdeskTicketReq {
    /// ticket id
    ///
    /// **是否必填**: 是
    ///
...
struct GetHelpdeskTicketRespInner {
    #[serde(flatten)]
    data: Option<GetHelpdeskTicketResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct GetHelpdeskTicketResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(GetHelpdeskTicketReq) -> Result<(GetHelpdeskTicketResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(GetHelpdeskTicketReq) -> Result<(GetHelpdeskTicketResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> HelpdeskServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_helpdesk_ticket<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, GetHelpdeskTicketReq, GetHelpdeskTicketResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_get_helpdesk_ticket(
            &self,
            req: &GetHelpdeskTicketReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, GetHelpdeskTicketReq, GetHelpdeskTicketResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::helpdesk::get_helpdesk_ticket::{
            GetHelpdeskTicketReq, GetHelpdeskTicketResp, GetHelpdeskTicketRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .helpdesk()
            .mock()
            .mock_get_helpdesk_ticket(|_| {
                Ok((GetHelpdeskTicketResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .helpdesk()
            .get_helpdesk_ticket(GetHelpdeskTicketReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetHelpdeskTicketRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
