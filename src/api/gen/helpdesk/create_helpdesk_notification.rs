//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/helpdesk-v1/notification/create>
...
impl<'c, IStore: Store, IClient: HttpClient> HelpdeskService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/helpdesk-v1/notification/create>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/helpdesk-v1/notification/create>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fhelpdesk-v1%2Fnotification%2Fcreate>
    pub async fn create_helpdesk_notification(
        &self,
        req: CreateHelpdeskNotificationReq,
    ) -> Result<(CreateHelpdeskNotificationResp, CommonResponse), Error> {
        self.create_helpdesk_notification_with_opt(req, Default::default())
            .await
    }

    /// 参见 [create_helpdesk_notification](#method.create_helpdesk_notification) 函数
    pub async fn create_helpdesk_notification_with_opt(
        &self,
        req: CreateHelpdeskNotificationReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_create_helpdesk_notification(&req) {
                tracing::info!("[lark] Helpdesk#CreateHelpdeskNotification **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Helpdesk#CreateHelpdeskNotification call api");
        ...
        let (resp, common_resp): (CreateHelpdeskNotificationRespInner, _) =
...
pub struct CreateHelpdeskNotificationReq {
    /// 用户 ID 类型
    ///
    /// **示例值**: "open_id"
    ///
...
struct CreateHelpdeskNotificationRespInner {
    #[serde(flatten)]
    data: Option<CreateHelpdeskNotificationResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct CreateHelpdeskNotificationResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    impl<'c, IStore: Store, IClient: HttpClient> HelpdeskServiceMocker<'c, IStore, IClient> {
        pub fn mock_create_helpdesk_notification<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<
            Mocker,
        ...
        pub(super) fn get_mock_create_helpdesk_notification(
            &self,
            req: &CreateHelpdeskNotificationReq,
...
mod test {
    use crate::{
        api::gen::helpdesk::create_helpdesk_notification::{
            CreateHelpdeskNotificationReq, CreateHelpdeskNotificationResp,
            CreateHelpdeskNotificationRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .helpdesk()
            .mock()
            .mock_create_helpdesk_notification(|_| {
                Ok((
                    CreateHelpdeskNotificationResp::default(),
        ...
        let res = lark
            .helpdesk()
            .create_helpdesk_notification(CreateHelpdeskNotificationReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::CreateHelpdeskNotificationReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<CreateHelpdeskNotificationRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
