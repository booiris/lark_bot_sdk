//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/location/get>
...
impl<'c, IStore: Store, IClient: HttpClient> CoreHrService<'c, IStore, IClient> {
    /// **api 版本: 2024-05-10T06:43:44+00:00**
    ///
    /// ## 查询单个地点
    ///
    /// 根据 ID 查询单个地点。
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/corehr-v1/location/get>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/corehr-v1/organization-management/location/get>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fcorehr-v1%2Forganization-management%2Flocation%2Fget>
    pub async fn get_core_hr_location(
        &self,
        req: GetCoreHrLocationReq,
    ) -> Result<(GetCoreHrLocationResp, CommonResponse), Error> {
        self.get_core_hr_location_with_opt(req, Default::default())
            .await
    }

    /// 参见 [get_core_hr_location](#method.get_core_hr_location) 函数
    pub async fn get_core_hr_location_with_opt(
        &self,
        req: GetCoreHrLocationReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_get_core_hr_location(&req) {
                tracing::info!("[lark] CoreHr#GetCoreHrLocation **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] CoreHr#GetCoreHrLocation call api");
        ...
        let (resp, common_resp): (GetCoreHrLocationRespInner, _) = self.cli.do_req(req).await?;
...
pub struct GetCoreHrLocationReq {
    /// 地点 ID
    ///
    /// **是否必填**: 是
    ///
    /// **示例值**: "1215"
    #[api(kind = "path", name = "location_id")]
    pub location_id: String,
}

#[derive(Debug, Serialize, Deserialize, Clone, lark_bot_sdk_macros::ApiBaseResp)]
struct GetCoreHrLocationRespInner {
    #[serde(flatten)]
    data: Option<GetCoreHrLocationResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct GetCoreHrLocationResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(GetCoreHrLocationReq) -> Result<(GetCoreHrLocationResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(GetCoreHrLocationReq) -> Result<(GetCoreHrLocationResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> CoreHrServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_core_hr_location<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, GetCoreHrLocationReq, GetCoreHrLocationResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_get_core_hr_location(
            &self,
            req: &GetCoreHrLocationReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, GetCoreHrLocationReq, GetCoreHrLocationResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::core_hr::get_core_hr_location::{
            GetCoreHrLocationReq, GetCoreHrLocationResp, GetCoreHrLocationRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .core_hr()
            .mock()
            .mock_get_core_hr_location(|_| {
                Ok((GetCoreHrLocationResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .core_hr()
            .get_core_hr_location(GetCoreHrLocationReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetCoreHrLocationRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
