//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-field/create>
...
impl<'c, IStore: Store, IClient: HttpClient> BitableService<'c, IStore, IClient> {
    /// **api 版本: 2024-05-14T03:22:45+00:00**
    ///
    /// ## 新增字段
    ///
    /// 该接口用于在数据表中新增一个字段
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-field/create>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/docs/bitable-v1/app-table-field/create>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fdocs%2Fbitable-v1%2Fapp-table-field%2Fcreate>
    pub async fn create_bitable_field(
        &self,
        req: CreateBitableFieldReq,
    ) -> Result<(CreateBitableFieldResp, CommonResponse), Error> {
        self.create_bitable_field_with_opt(req, Default::default())
            .await
    }

    /// 参见 [create_bitable_field](#method.create_bitable_field) 函数
    pub async fn create_bitable_field_with_opt(
        &self,
        req: CreateBitableFieldReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_create_bitable_field(&req) {
                tracing::info!("[lark] Bitable#CreateBitableField **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Bitable#CreateBitableField call api");
        ...
        let (resp, common_resp): (CreateBitableFieldRespInner, _) = self.cli.do_req(req).await?;
...
pub struct CreateBitableFieldReq {
...
struct CreateBitableFieldRespInner {
    #[serde(flatten)]
    data: Option<CreateBitableFieldResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct CreateBitableFieldResp {
    /// \-
    #[serde(
        rename = "data",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(CreateBitableFieldReq) -> Result<(CreateBitableFieldResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(CreateBitableFieldReq) -> Result<(CreateBitableFieldResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> BitableServiceMocker<'c, IStore, IClient> {
        pub fn mock_create_bitable_field<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, CreateBitableFieldReq, CreateBitableFieldResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_create_bitable_field(
            &self,
            req: &CreateBitableFieldReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, CreateBitableFieldReq, CreateBitableFieldResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::bitable::create_bitable_field::{
            CreateBitableFieldReq, CreateBitableFieldResp, CreateBitableFieldRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .bitable()
            .mock()
            .mock_create_bitable_field(|_| {
                Ok((CreateBitableFieldResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .bitable()
            .create_bitable_field(CreateBitableFieldReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::CreateBitableFieldReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<CreateBitableFieldRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
