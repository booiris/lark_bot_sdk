//! doc url: <https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list>
...
impl<'c, IStore: Store, IClient: HttpClient> DriveService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/docs/docs/docx-v1/document/list>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fdocs%2Fdocs%2Fdocx-v1%2Fdocument%2Flist>
    pub async fn get_docx_block_list_of_document(
        &self,
        req: GetDocxBlockListOfDocumentReq,
    ) -> Result<(GetDocxBlockListOfDocumentResp, CommonResponse), Error> {
        self.get_docx_block_list_of_document_with_opt(req, Default::default())
            .await
    }

    /// 参见 [get_docx_block_list_of_document](#method.get_docx_block_list_of_document) 函数
    pub async fn get_docx_block_list_of_document_with_opt(
        &self,
        req: GetDocxBlockListOfDocumentReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_get_docx_block_list_of_document(&req) {
                tracing::info!("[lark] Drive#GetDocxBlockListOfDocument **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Drive#GetDocxBlockListOfDocument call api");
        ...
        let (resp, common_resp): (GetDocxBlockListOfDocumentRespInner, _) =
...
pub struct GetDocxBlockListOfDocumentReq {
    /// 文档的唯一标识。对应新版文档 Token，[点击了解如何获取云文档 Token](https://open.feishu.cn/document/ukTMukTMukTM/uczNzUjL3czM14yN3MTN#08bb5df6)。
    ///
    /// **是否必填**: 是
    ///
...
struct GetDocxBlockListOfDocumentRespInner {
    #[serde(flatten)]
    data: Option<GetDocxBlockListOfDocumentResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct GetDocxBlockListOfDocumentResp {
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    impl<'c, IStore: Store, IClient: HttpClient> DriveServiceMocker<'c, IStore, IClient> {
        pub fn mock_get_docx_block_list_of_document<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<
            Mocker,
        ...
        pub(super) fn get_mock_get_docx_block_list_of_document(
            &self,
            req: &GetDocxBlockListOfDocumentReq,
...
mod test {
    use crate::{
        api::gen::drive::get_docx_block_list_of_document::{
            GetDocxBlockListOfDocumentReq, GetDocxBlockListOfDocumentResp,
            GetDocxBlockListOfDocumentRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .drive()
            .mock()
            .mock_get_docx_block_list_of_document(|_| {
                Ok((
                    GetDocxBlockListOfDocumentResp::default(),
        ...
        let res = lark
            .drive()
            .get_docx_block_list_of_document(GetDocxBlockListOfDocumentReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<GetDocxBlockListOfDocumentRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
