//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/thread/forward>
...
impl<'c, IStore: Store, IClient: HttpClient> MessageService<'c, IStore, IClient> {
    /// **api 版本: 2024-06-06T08:42:06+00:00**
    ///
    /// ## 转发话题
    ///
    /// 向用户、群聊或话题转发一个话题。
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/thread/forward>
    ///
    /// new doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/thread/forward>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2FuAjLw4CM%2FukTMukTMukTM%2Freference%2Fim-v1%2Fthread%2Fforward>
    pub async fn forward_thread_message(
        &self,
        req: ForwardThreadMessageReq,
    ) -> Result<(ForwardThreadMessageResp, CommonResponse), Error> {
        self.forward_thread_message_with_opt(req, Default::default())
            .await
    }

    /// 参见 [forward_thread_message](#method.forward_thread_message) 函数
    pub async fn forward_thread_message_with_opt(
        &self,
        req: ForwardThreadMessageReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_forward_thread_message(&req) {
                tracing::info!("[lark] Message#ForwardThreadMessage **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Message#ForwardThreadMessage call api");
        ...
        let (resp, common_resp): (ForwardThreadMessageRespInner, _) = self.cli.do_req(req).await?;
...
pub struct ForwardThreadMessageReq {
    /// 要转发的话题ID，详见：[话题介绍](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/thread-introduction)
    ///
    /// </md-enum-item>
    ///
    /// **是否必填**: 是
    ...
    ///
    /// `UserID`: 标识一个用户在某个租户内的身份。同一个用户在租户 A 和租户 B 内的 User ID 是不同的。在同一个租户内，一个用户的 User ID 在所有应用（包括商店应用）中都保持一致。User ID 主要用于在不同的应用间打通用户数据。
    ///
    ...
    /// `Email`: 以用户的真实邮箱来标识用户。
    ///
    /// `ChatID`: 以群ID来标识群聊。[了解更多：如何获取群ID ](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/chat-id-description)
    ///
    /// `ThreadID`: 以话题ID来标识话题。了解更多：[话题介绍](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/thread-introduction)
    ///
    /// **当值为 `user_id`，字段权限要求**：
    ///
    /// (**仅自建应用**) 获取用户 user ID
...
struct ForwardThreadMessageRespInner {
    #[serde(flatten)]
    data: Option<ForwardThreadMessageResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct ForwardThreadMessageResp {
    /// \-
    #[serde(
        rename = "data",
...
pub struct MessageBodySubResp {
    ...
    /// **是否必填**: 是
    ///
    /// **示例值**: "Merged and Forwarded Message"
    #[serde(
        rename = "content",
        deserialize_with = "crate::utils::serde_helper::null_to_default"
    )]
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(ForwardThreadMessageReq) -> Result<(ForwardThreadMessageResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<'c, IStore: Store, IClient: HttpClient> MessageServiceMocker<'c, IStore, IClient> {
        pub fn mock_forward_thread_message<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<
            Mocker,
        ...
        pub(super) fn get_mock_forward_thread_message(
            &self,
            req: &ForwardThreadMessageReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, ForwardThreadMessageReq, ForwardThreadMessageResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::message::forward_thread_message::{
            ForwardThreadMessageReq, ForwardThreadMessageResp, ForwardThreadMessageRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .message()
            .mock()
            .mock_forward_thread_message(|_| {
                Ok((
                    ForwardThreadMessageResp::default(),
        ...
        let res = lark
            .message()
            .forward_thread_message(ForwardThreadMessageReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::ForwardThreadMessageReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<ForwardThreadMessageRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
