//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/reply>
...
impl<'c, IStore: Store, IClient: HttpClient> MessageService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/reply>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/im-v1/message/reply>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fim-v1%2Fmessage%2Freply>
    pub async fn reply_raw_message(
        &self,
        req: ReplyRawMessageReq,
    ) -> Result<(ReplyRawMessageResp, CommonResponse), Error> {
        self.reply_raw_message_with_opt(req, Default::default())
            .await
    }

    /// 参见 [reply_raw_message](#method.reply_raw_message) 函数
    pub async fn reply_raw_message_with_opt(
        &self,
        req: ReplyRawMessageReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_reply_raw_message(&req) {
                tracing::info!("[lark] Message#ReplyRawMessage **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Message#ReplyRawMessage call api");
        ...
        let (resp, common_resp): (ReplyRawMessageRespInner, _) = self.cli.do_req(req).await?;
...
pub struct ReplyRawMessageReq {
    ...
    /// 消息内容 json 格式，格式说明参考: [发送消息内容](https://open.feishu.cn/document/uAjLw4CM/ukTMukTMukTM/im-v1/message/create_json)
    ///
    /// **是否必填**: 是
    ///
    ...
    ///
    /// **示例值**: "false"
    #[api(kind = "body", name = "reply_in_thread")]
...
struct ReplyRawMessageRespInner {
    #[serde(flatten)]
    data: Option<ReplyRawMessageResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct ReplyRawMessageResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(ReplyRawMessageReq) -> Result<(ReplyRawMessageResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(ReplyRawMessageReq) -> Result<(ReplyRawMessageResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> MessageServiceMocker<'c, IStore, IClient> {
        pub fn mock_reply_raw_message<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, ReplyRawMessageReq, ReplyRawMessageResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_reply_raw_message(
            &self,
            req: &ReplyRawMessageReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, ReplyRawMessageReq, ReplyRawMessageResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::message::reply_raw_message::{
            ReplyRawMessageReq, ReplyRawMessageResp, ReplyRawMessageRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .message()
            .mock()
            .mock_reply_raw_message(|_| {
                Ok((ReplyRawMessageResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .message()
            .reply_raw_message(ReplyRawMessageReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::ReplyRawMessageReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<ReplyRawMessageRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
