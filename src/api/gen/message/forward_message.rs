//! doc url: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/forward>
...
impl<'c, IStore: Store, IClient: HttpClient> MessageService<'c, IStore, IClient> {
    ...
    ///
    /// doc: <https://open.larkoffice.com/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/forward>
    ///
    /// new doc: <https://open.larkoffice.com/document/server-docs/im-v1/message/forward>
    ///
    /// api url: <https://open.feishu.cn/document_portal/v1/document/get_detail?fullPath=%2Fserver-docs%2Fim-v1%2Fmessage%2Fforward>
    pub async fn forward_message(
        &self,
        req: ForwardMessageReq,
    ) -> Result<(ForwardMessageResp, CommonResponse), Error> {
        self.forward_message_with_opt(req, Default::default()).await
    }

    /// 参见 [forward_message](#method.forward_message) 函数
    pub async fn forward_message_with_opt(
        &self,
        req: ForwardMessageReq,
        ...
        {
            if let Some(f) = self.mock().get_mock_forward_message(&req) {
                tracing::info!("[lark] Message#ForwardMessage **mocking** api");
                return f(req);
            }
        }

        tracing::info!("[lark] Message#ForwardMessage call api");
        ...
        let (resp, common_resp): (ForwardMessageRespInner, _) = self.cli.do_req(req).await?;
...
struct ForwardMessageRespInner {
    #[serde(flatten)]
    data: Option<ForwardMessageResp>,
    #[serde(flatten)]
    base: BaseResp,
}

#[derive(Debug, Serialize, Deserialize, Clone, Default)]
pub struct ForwardMessageResp {
    /// \-
    #[serde(
        rename = "data",
...
mod test_utils {
    ...
    pub trait MockFunc:
        Fn(ForwardMessageReq) -> Result<(ForwardMessageResp, CommonResponse), Error>
        + Send
        + Sync
        + 'static
    {
    ...
    impl<
            T: Fn(ForwardMessageReq) -> Result<(ForwardMessageResp, CommonResponse), Error>
                + Send
                + Sync
                + 'static,
        > MockFunc for T
    ...
    impl<'c, IStore: Store, IClient: HttpClient> MessageServiceMocker<'c, IStore, IClient> {
        pub fn mock_forward_message<F: MockFunc>(
            &self,
            f: F,
        ) -> MockerBuilder<Mocker, ForwardMessageReq, ForwardMessageResp, Arc<dyn MockFunc>>
        {
            MockerBuilder::new(self.cli.instance_id, Arc::new(f))
        }

        pub(super) fn get_mock_forward_message(
            &self,
            req: &ForwardMessageReq,
        ) -> Option<Arc<dyn MockFunc>> {
            do_mock::<Mocker, ForwardMessageReq, ForwardMessageResp, Arc<dyn MockFunc>>(
...
mod test {
    use crate::{
        api::gen::message::forward_message::{
            ForwardMessageReq, ForwardMessageResp, ForwardMessageRespInner,
        },
    ...
    async fn test_mock() {
        ...
        let mocker = lark
            .message()
            .mock()
            .mock_forward_message(|_| {
                Ok((ForwardMessageResp::default(), CommonResponse::default()))
            })
            .build();
        let res = lark
            .message()
            .forward_message(ForwardMessageReq::default())
            .await;
        assert!(res.is_ok());
        mocker.clear();
        let res = lark
    ...
    fn test_req() {
        ...
        if let Err(e) = serde_json::from_str::<super::ForwardMessageReqBody>(REQ) {
            panic!("{}", e);
        }
    }
    ...
    #[test]
    fn test_resp() {
        let res = serde_json::from_str::<ForwardMessageRespInner>(RESP);
        if let Err(e) = res {
            panic!("{}", e);
        }
